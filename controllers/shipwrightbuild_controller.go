...
func (r *ShipwrightBuildReconciler) setFinalizer(
	ctx context.Context,
...
func (r *ShipwrightBuildReconciler) unsetFinalizer(
	ctx context.Context,
	b *v1alpha1.ShipwrightBuild,
) error {
	finalizers := []string{}
...
func (r *ShipwrightBuildReconciler) Reconcile(
	ctx context.Context,
	req ctrl.Request,
) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, b); err != nil {
		...
		return NoRequeueWithError(err)
	}

	// selecting the target namespace based on the CRD information, when not informed directly it
	// falls back to to the same namespace than the actual CRD object is located
